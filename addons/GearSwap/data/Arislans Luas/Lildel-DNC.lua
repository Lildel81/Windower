...
function job_setup()
	...
	state.Buff['Stricking Flourish'] = buffactive['Striking Florish'] or false
    state.Buff['Sneak Attack'] = buffactive['sneak attack'] or false
	state.Buff['Aftermath: Lv.3'] = buffactive['Aftermath: lvl. 3'] or false
...
--  state.SkillchainPending = M(false, 'Skillchain Pending')
	...
	state.MaxAttack = M(true, "Max Attack")
...
function user_setup()
    state.OffenseMode:options('Normal', 'LowAcc', 'MidAcc', 'HighAcc', 'STP', 'MaxAttack')
...
function init_gear_sets()
	...
	sets.precast.WS['Pyrrhic Kleos'].Max = {ammo="Staunch Tathlum",
	...
	sets.MaxAttack = {ammo="Aurgelmir Orb +1",
    head="Malignance Chapeau",
    body="Ashera Harness",
    hands="Malignance Gloves",
    legs={ name="Samnuha Tights", augments={'STR+10','DEX+10','"Dbl.Atk."+3','"Triple Atk."+3',}},
	...
	sets.SC = {hands={ name="Nyame Gauntlets", augments={'Path: B',}},
    legs="Maxixi Tights +2",
    neck={ name="Warder's Charm +1", augments={'Path: A',}},}
--916-660-7340     916-660-7250
    ------------------------------------------------------------------------------------------------
    ---------------------------------------- Midcast Sets ------------------------------------------
    ------------------------------------------------------------------------------------------------
    ...
    sets.engaged.DW.STP = set_combine(sets.engaged.DW, {ammo="Aurgelmir Orb +1",
    head="Malignance Chapeau",
    body="Ashera Harness",
    hands="Malignance Gloves",
    legs={ name="Samnuha Tights", augments={'STR+10','DEX+10','"Dbl.Atk."+3','"Triple Atk."+3',}},
    ...
    sets.engaged.DW.STP.LowHaste = set_combine(sets.engaged.DW.LowHaste, {ammo="Aurgelmir Orb +1",
    head="Malignance Chapeau",
    body="Ashera Harness",
    hands="Malignance Gloves",
    legs={ name="Samnuha Tights", augments={'STR+10','DEX+10','"Dbl.Atk."+3','"Triple Atk."+3',}},
    ...
    sets.engaged.DW.STP.MidHaste = set_combine(sets.engaged.DW.MidHaste, {ammo="Aurgelmir Orb +1",
    head="Malignance Chapeau",
    body="Ashera Harness",
    hands="Malignance Gloves",
    legs={ name="Samnuha Tights", augments={'STR+10','DEX+10','"Dbl.Atk."+3','"Triple Atk."+3',}},
    ...
    sets.engaged.DW.STP.HighHaste = set_combine(sets.engaged.DW.HighHaste, {ammo="Aurgelmir Orb +1",
    head="Malignance Chapeau",
    body="Ashera Harness",
    hands="Malignance Gloves",
    legs={ name="Samnuha Tights", augments={'STR+10','DEX+10','"Dbl.Atk."+3','"Triple Atk."+3',}},
    ...
    sets.engaged.DW.STP.MaxHaste = set_combine(sets.engaged.DW.MaxHaste, {ammo="Aurgelmir Orb +1",
    head="Malignance Chapeau",
    body="Ashera Harness",
    hands="Malignance Gloves",
    legs={ name="Samnuha Tights", augments={'STR+10','DEX+10','"Dbl.Atk."+3','"Triple Atk."+3',}},
...
function job_precast(spell, action, spellMap, eventArgs)
	...
	--[[if spell.type == "WeaponSkill" then
		if state.MaxAttack then 
			equip (sets.MaxAttack)
...
function job_post_precast(spell, action, spellMap, eventArgs)
    if spell.type == "WeaponSkill" then
		...
		if state.Buff['Striking Florish'] then
			equip(sets.buff['Striking Flourish'])
		end
			
		if state.WeaponskillMode.current == 'SC' then
			if spell.type == 'WeaponSkill' then
				add_to_chat(167, 'Time Elapsed == ' ..(os.time()-skillChainTimer))
...
--    end
	...
	if buff == "Saber Dance" and not gain then
		add_to_chat(167, 'Saber Dance just Expired!!')
		if state.autoSaberDance == true then
			send_command('@input /ja "Saber Dance" <me>')
		end
	end
	--[[if buff == "Aftermath: Lv.3" and gain then
		if player.equipment.main == 'Terpsichore' then
			add_to_chat(167, 'Setting offence to STP')
			send_command('gs c set offensemode stp')
		end
	else
		handle_equipping_gear(player.status)
...
function get_custom_wsmode(spell, action, spellMap)
	...
	if state.OffenseMode.value == "MaxAttack" then
		wsmode = 'Max'
	end
...
function customize_melee_set(meleeSet, buff)
    --if state.Buff['Climactic Flourish'] then
	...
	--elseif buff == 'Aftermath: Lvl.3' then
		--meleeSet = sets.AM3
	end
	if state.Buff['Aftermath: Lv.3'] then
			meleeSet = sets.buff['AM']
		end

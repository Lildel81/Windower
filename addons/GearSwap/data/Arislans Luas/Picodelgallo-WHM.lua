...
function get_sets()
	...
	include('Picodelgallo_Globals.lua')
end
...
function user_setup()
	...
	state.GI = M(true)
	state.AutoSubMode = M(false, 'AutoSubMode')
	send_command('bind ^s gs c toggle autosubmode')
	state.Buffup = false
...
function init_gear_sets()
    ...
    hands={ name="Gendewitha Gages", augments={'Phys. dmg. taken -3%','"Cure" potency +4%',}}, -- 5
    legs="Assiduity Pants",
    feet="Regal Pumps +1", -- 7
    neck={ name="Clr. Torque +2", augments={'Path: A',}},--4
    waist="Embla Sash", -- 5
    ...
    body="Inyanga Jubbah", --10
    hands={ name="Gendewitha Gages", augments={'Phys. dmg. taken -3%','"Cure" potency +4%',}}, --5
    legs="Ebers Pantaloons +2", --12
    feet="Hygieia Clogs", --17
    neck={ name="Clr. Torque +2", augments={'Path: A',}}, --4
    waist="Embla Sash", --5
    left_ear="Mendi. Earring", --5
    right_ear="Nourish. Earring", --3
    left_ring="Prolix Ring", --2
    right_ring="Kishar Ring", --4
        }) --/sch 90
    ...
    sets.midcast.Cursna = set_combine(sets.midcast.StatusRemoval, {
        main={ name="Gada", augments={'"Fast Cast"+5','Mag. Acc.+19','DMG:+19',}},
    sub="Sors Shield",
    head={ name="Vanya Hood", augments={'MP+50','"Fast Cast"+10','Haste+2%',}},
    body={ name="Kaykaus Bliaut +1", augments={'MP+80','"Cure" potency +6%','"Conserve MP"+7',}},
    ...
    sub="Sors Shield",
    head="Befouled Crown",
    body={ name="Telchine Chas.", augments={'Pet: "Regen"+2','Pet: Damage taken -3%',}},
...
if state.AutoSubMode == true then
    if not (buffactive['Sublimation: Activated'] or buffactive['Sublimation: Complete']) then
        if not (buffactive['Invisible'] or buffactive['Weakness']) then
            if abil_recasts[234] == 0 then
                send_command('wait 5;input /ja "Sublimation" <me>')
            end
        end
    elseif buffactive['Sublimation: Complete'] then
        if player.mpp < 20 and abil_recasts[234] == 0 then
            send_command('wait 5;input /ja "Sublimation" <me>')
        end
	elseif buffactive['Sublimation: Activated'] then
		if player.mpp < 10 and abil_recasts[234] == 0 then
			send_command('wait 5; input /ja "Sublimation" <me>')
		end
    end
end
...
-------------------------------------------------------------------------------------------------------------------
function reraiseWore()
	add_to_chat(167, '!!!!!!!! RERAISE EXPIRED RECAST NOW !!!!!!!!!!')
end
